...
describe("cosineSimilarity", () => {
  it("is 1 for identical direction, 0 for orthogonal", () => {
    expect(
      cosineSimilarity(new Float32Array([1, 0]), new Float32Array([2, 0])),
    ).toBeCloseTo(1);
    expect(
...
describe("retrieve", () => {
  ...
  it("ranks by cosine similarity and returns top-k citations", () => {
    ...
    expect(citations.map((m) => m.transcript)).toEqual([
      "aligned",
      "orthogonal",
    ]);
  });
  ...
  it("skips memos without a usable embedding", () => {
    const withEmpty: VoiceMemo[] = [
      memo("aligned", [1, 0]),
      {
    ...
    const { citations } = retrieve(query, withEmpty, 5);
    expect(citations.map((m) => m.transcript)).toEqual(["aligned"]);
  });
});
